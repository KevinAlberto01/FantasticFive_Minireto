    ...
    try:
        ...
        for marker_data in stub.DetectMarkers(empty_request):
            new_marker_data[marker_data.id] = {
                "id": marker_data.id,
                "corners": list(marker_data.corners.corners),  # Convertir a lista
                "center": list(marker_data.center.center)  # Convertir a lista
            }
        if new_marker_data:
...
if __name__ == '__main__':
    ...
    schedule.every(5).seconds.do(call_api_image)  # Reducir la frecuencia de llamadas
